...
//			logger.error("failed to access storage: {}", e.getMessage());
	...
	public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx) {
		...
		if (eth.isBroadcast() == true) {
			System.out.println("eth.isbroadcast-----");
		...
		if (decision == null) {
			...
			if (rule == null || rule.action == FirewallRule.FirewallAction.DROP) {
				...
				if (rule == null) {
					...
					Debug.printPC("drop packet pc");
					System.out.println("No firewall rule found for PacketIn={}"+pi);
					System.out.println(decision.toString());
					System.out.println(Debug.getSymbolicIntegerValue(decision.getMatch().get(MatchField.IPV4_SRC).getInt()));
					System.out.println(Debug.getSymbolicIntegerValue(decision.getMatch().get(MatchField.IPV4_DST).getInt()));
				} else {
					decision.setDescriptor(AppCookie.makeCookie(APP_ID, rule.ruleid));
					Debug.printPC("drop packet pc");
					System.out.println( "deny rule found for PacketIn={}"+pi+rule);
					System.out.println("**************"+decision.toString());
					System.out.println(Debug.getSymbolicIntegerValue(decision.getMatch().get(MatchField.IPV4_SRC).getInt()));
					System.out.println(Debug.getSymbolicIntegerValue(decision.getMatch().get(MatchField.IPV4_DST).getInt()));
				}
//				decision.addToContext(cntx);
////				Debug.printPC("drop packet pc");
//				System.out.println("No firewall rule or deny rule found for PacketIn={}"+pi);
//				System.out.println(decision.toString()); 
////				if (logger.isTraceEnabled()) {
//					if (rule == null) {
...
//				}
				...
				decision.setDescriptor(AppCookie.makeCookie(APP_ID, rule.ruleid));
				decision.addToContext(cntx);
				Debug.printPC("alow packet pc");
				System.out.println("Allow rule={} match for PacketIn={}"+rule+pi);
				System.out.println(decision.toString());
				System.out.println(Debug.getSymbolicIntegerValue(decision.getMatch().get(MatchField.IPV4_SRC).getInt()));
				System.out.println(Debug.getSymbolicIntegerValue(decision.getMatch().get(MatchField.IPV4_DST).getInt()));
//				if (logger.isTraceEnabled()) {
